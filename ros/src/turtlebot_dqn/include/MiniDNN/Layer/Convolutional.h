...
public:
    ...
    void init_normal(const Scalar& mu, const Scalar& sigma, RNG& rng)
    {
        // Set data dimension
    ...
    void init_uniform(const Scalar& a, const Scalar& b, RNG& rng)
    {
        // Set data dimension
        const int filter_data_size = m_dim.in_channels * m_dim.out_channels * m_dim.filter_rows * m_dim.filter_cols;
        m_filter_data.resize(filter_data_size);
        ...
        // Random initialization of filter parameters
        internal::set_uniform_random(m_filter_data.data(), filter_data_size, rng, a, b);

        // Bias term
        m_bias.resize(m_dim.out_channels);
        m_db.resize(m_dim.out_channels);
        internal::set_uniform_random(m_bias.data(), m_dim.out_channels, rng, a, b);
    }
