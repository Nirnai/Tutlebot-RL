...
public:
    ...
    ///
    void init_normal(const Scalar& mu = Scalar(0), const Scalar& sigma = Scalar(0.01), int seed = -1)
    {
        check_unit_sizes();
        ...
        for(int i = 0; i < nlayer; i++)
        {
            m_layers[i]->init_normal(mu, sigma, m_rng);
        }
    }
    ...
    ///              use the current random state.
    ///
    void init_uniform(const Scalar& a = Scalar(0), const Scalar& b = Scalar(1), int seed = -1)
    {
        check_unit_sizes();
        ...
        for(int i = 0; i < nlayer; i++)
        {
            m_layers[i]->init_uniform(a, b, m_rng);
        }
    }
